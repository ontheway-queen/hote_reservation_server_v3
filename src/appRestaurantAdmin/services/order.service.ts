		...
		return await this.db.transaction(async (trx) => {
			...
			for (const item of order_items) {
				...
				if (food.ingredients === null) {
					throw new CustomError(
						"No ingredients found for this food item.",
						this.StatusCode.HTTP_NOT_FOUND
					);
				}
	...
	public async completeOrderPayment(req: Request) {
		return await this.db.transaction(async (trx) => {
			...
			if (pay_with === "reservation") {
				...
				if (room_id) {
					const [singleRoom] =
						await this.Model.RoomModel().getSingleRoom(
							hotel_code,
							room_id
						);
					if (!singleRoom) {
					...
					if (!is_individual_booking) {
						...
						const primaryFolio =
							await hotelInvModel.getFoliosbySingleBooking({
								booking_id,
								hotel_code,
								type: "Primary",
							});
	...
	public async updateOrder(req: Request) {
		return await this.db.transaction(async (trx) => {
			...
			const serviceChargeAmount = Lib.calculatePercentageToAmount(
				...
				rest.service_charge ?? Number(existingOrder.service_charge),
				rest.service_charge_type ??
					(existingOrder.service_charge_type as
						| "percentage"
						| "fixed")
			);
			...
			const vatAmount = Lib.calculatePercentageToAmount(
				...
				rest.vat ?? Number(existingOrder.vat),
				rest.vat_type ??
					(existingOrder.vat_type as "percentage" | "fixed")
			);
			...
			if (order_type === "walk-in") {
				...
				if (!booking_id) {
					return {
						...
						code: this.StatusCode.HTTP_BAD_REQUEST,
						message:
							"Please provide booking ID for reservation guest.",
					};
				}
			...
			await restaurantOrderModel.updateOrder({
				...
				payload: {
					...
					grand_total,
					kitchen_status:
						rest.kitchen_status ?? existingOrder.kitchen_status,
					updated_by: id,
					discount_amount: discountAmount,
					service_charge_amount: serviceChargeAmount,
					vat_amount: vatAmount,
