...
export class RoomService extends AbstractServices {
	...
	public async createMultipleRooms(req: Request) {
		return await this.db.transaction(async (trx) => {
			...
			const creationResults = await Promise.all(roomPromises);
			const createdRoomNames = creationResults.filter(
				(name) => !!name
			) as string[];
			...
			if (createdCount > 0) {
				...
				if (availability) {
					await roomModel.updateInRoomAvailabilities(
						...
						{
							available_rooms:
								availability.available_rooms + createdCount,
							total_rooms:
								availability.total_rooms + createdCount,
						}
					);
				} else {
