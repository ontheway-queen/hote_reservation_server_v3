...
import { STOCK_STATUS } from "../../utils/miscellaneous/constants";
	...
	public async createStock(req: Request) {
		return await this.db.transaction(async (trx) => {
			...
			if (stock_in) {
				...
				const purchase_product_ids = stock_items.map(
					...
					if (inventoryItem) {
						modifyInventoryProduct.push({
							available_quantity:
								...
								payloadItem.quantity,
							quantity_used:
								parseFloat(inventoryItem.quantity_used) +
								payloadItem.quantity,
							id: inventoryItem.id,
						});
					}
