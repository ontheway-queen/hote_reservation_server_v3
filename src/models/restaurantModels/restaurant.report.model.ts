...
`
	...
	public async getSellsReport(query: {
      ...
      `)
		...
		const typeWiseSales = await this.db
			...
			.select(
				"o.order_type",
				this.db.raw("SUM(o.net_total) as net_total_amount")
			)
			.from("orders as o")
			.where("o.status", "completed")
			.andWhere("o.hotel_code", hotel_code)
	...
	public async getProductsReport(query: {
		...
		return await this.db
			...
			.select(
				...
				this.db.raw(
					"COALESCE(SUM(oi.quantity * oi.rate), 0) as total_sales"
				)
			)
			.from("order_items as oi")
	...
	public async getUserSellsReport(query: {
		...
		const { from_date, to_date, hotel_code, restaurant_id, user_id } =
			query;
