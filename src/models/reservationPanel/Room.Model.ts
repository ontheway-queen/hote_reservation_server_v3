...
class RoomModel extends Schema {
  ...
  public async getAllOccupiedRooms(date: string): Promise<{
    data: IOccupiedRoom[];
    total: number;
  }> {
    const data = await this.db("booking_rooms as br")
      .withSchema(this.RESERVATION_SCHEMA)
      .select(
        "br.id",
        "br.adults",
        ...
        "bk.total_nights",
        "bk.status",
        this.db.raw(`TO_CHAR(bk.check_in, 'YYYY-MM-DD') as check_in`),
        this.db.raw(`TO_CHAR(bk.check_out, 'YYYY-MM-DD') as check_out`),
        this.db.raw(`TO_CHAR(bk.booking_date, 'YYYY-MM-DD') as booking_date`),
        "r.id as room_id",
        "r.room_name",
        "r.floor_no",
    ...
    const totalResult = await this.db("booking_rooms as br")
      .withSchema(this.RESERVATION_SCHEMA)
      .join("bookings as bk", "bk.id", "br.booking_id")
      .where((qb) => {
        qb.where("bk.status", "checked_in").orWhere("bk.status", "confirmed");
    ...
    return {
      total: parseInt(totalResult?.[0]?.total as string) || 0,
      data,
    };
  }
}
