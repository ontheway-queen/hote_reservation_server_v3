...
export class BtocReservationModel extends Schema {
  ...
  public async insertBooking(payload: IBtocBooking) {
    return await this.db("bookings")
      .withSchema(this.RESERVATION_SCHEMA)
      .insert(payload, "id");
  }

  public async insertBookingRoom(payload: IBtocbookingRooms[]) {
  ...
  public async getAllBooking({
    ...
    booking_type?: string;
    status?: string;
  }): Promise<{ total: number; data: IgetAllBooking[] }> {
    ...
    const total = await this.db("bookings")
      .withSchema(this.RESERVATION_SCHEMA)
      .count("id as total")
  ...
  public async cancelSingleBooking({
    hotel_code,
    ref_id,
    user_id,
  }: {
