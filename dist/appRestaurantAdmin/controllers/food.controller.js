...
class RestaurantFoodController extends abstract_controller_1.default {
    constructor() {
        ...
        this.wastageFood = this.asyncWrapper.wrap({
        ...
        this.getWastageFood = this.asyncWrapper.wrap({}, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _d = yield this.service.getWastageFood(req), { code } = _d, data = __rest(_d, ["code"]);
            res.status(code).json(data);
        }));
        this.createFoodV2 = this.asyncWrapper.wrap({ bodySchema: this.validator.createFoodV2Validator }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _e = yield this.service.createFoodV2(req), { code } = _e, data = __rest(_e, ["code"]);
            res.status(code).json(data);
        }));
        this.getFoods = this.asyncWrapper.wrap({ querySchema: this.validator.getFoodsValidator }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _f = yield this.service.getFoods(req), { code } = _f, data = __rest(_f, ["code"]);
            res.status(code).json(data);
        }));
        this.getFood = this.asyncWrapper.wrap({ paramSchema: this.commonValidator.singleParamStringValidator() }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _g = yield this.service.getFood(req), { code } = _g, data = __rest(_g, ["code"]);
            res.status(code).json(data);
        }));
        this.updateFood = this.asyncWrapper.wrap({
            ...
            const _h = yield this.service.updateFood(req), { code } = _h, data = __rest(_h, ["code"]);
            res.status(code).json(data);
        }));
        this.deleteFood = this.asyncWrapper.wrap({
            paramSchema: this.commonValidator.singleParamStringValidator(),
        }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _j = yield this.service.deleteFood(req), { code } = _j, data = __rest(_j, ["code"]);
            res.status(code).json(data);
        }));
        this.geFoodStocks = this.asyncWrapper.wrap({ querySchema: this.validator.getFoodStocksValidator }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _k = yield this.service.getFoodStocks(req), { code } = _k, data = __rest(_k, ["code"]);
            res.status(code).json(data);
        }));
    }
}
