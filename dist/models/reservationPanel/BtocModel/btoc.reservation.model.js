...
class BtocReservationModel extends schema_1.default {
    ...
    getAllRoomRatesBTOC(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            const { hotel_code, checkin, checkout, rooms, nights } = payload;
            const totalRequested = rooms.length;
            const roomTypes = yield this.db("channel_allocations as ca")
                .withSchema(this.CM_SCHEMA)
                .select("rt.id as room_type_id", "rt.name as room_type_name", "rt.description", "rt.max_adults", "rt.max_children", this.db.raw(`MIN(ca.allocated_rooms - ca.sold_rooms) AS available_count`), "rp.id as rate_plan_id", "rp.name as rate_plan_name", "rpd.base_rate", this.db.raw(`COALESCE(meals.meal_list, '[]')::json AS boarding_details`))
                .joinRaw("LEFT JOIN hotel_reservation.room_types as rt ON rt.id= ca.room_type_id")
                .joinRaw("LEFT JOIN hotel_reservation.rate_plan_details as rpd ON rt.id= rpd.room_type_id")
                .joinRaw("LEFT JOIN hotel_reservation.rate_plans as rp ON rpd.rate_plan_id=rp.id")
                .leftJoin(this.db
                .select("rpmp.rate_plan_id")
                .from("hotel_reservation.rate_plan_meal_mappings as rpmp")
                .leftJoin("hotel_reservation.meal_plans as mp", "rpmp.meal_plan_id", "mp.id")
