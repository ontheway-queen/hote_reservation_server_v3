...
class ReservationService extends abstract_service_1.default {
    ...
    getAllAvailableRooms(req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const nights = Math.ceil((new Date(check_out).getTime() - new Date(check_in).getTime()) /
                (1000 * 60 * 60 * 24));
            const totalRoomsRequested = rooms.length;
            const isAllSameConfig = rooms.every((r) => r.adults === rooms[0].adults && r.children === rooms[0].children);
            const db = this.db;
            const accomodationSetting = yield db("accommodation_settings")
                .withSchema(this.schema.RESERVATION_SCHEMA)
                .select("id", "has_child_rates", "check_in_time", "check_out_time")
                .where("hotel_code", hotel_code)
                .first();
            const childRateGroups = (accomodationSetting === null || accomodationSetting === void 0 ? void 0 : accomodationSetting.has_child_rates)
                ? yield db("child_rate_groups as crg")
                    .withSchema(this.schema.RESERVATION_SCHEMA)
                    .select("hotel_code", "id", "rate_plan_id", "age_from", "age_to", "rate_type", "rate_value", "room_type_id")
                    .where("crg.hotel_code", hotel_code)
                : [];
            // daily rates
            const fetchDailyRates = (roomTypeId, ratePlanId, checkIn, checkOut, hotelCode) => __awaiter(this, void 0, void 0, function* () {
                console.log({ roomTypeId, ratePlanId, checkIn, checkOut, hotelCode });
                return yield db("daily_rates as dr")
                    .withSchema(this.schema.RESERVATION_SCHEMA)
            ...
            const fetchMatchingRoomTypes = (adults, children) => __awaiter(this, void 0, void 0, function* () {
                return yield db("room_types as rt")
                    .withSchema(this.schema.RESERVATION_SCHEMA)
                    .select("rt.id as room_type_id", "rt.name as room_type_name", "rt.base_occupancy", "rt.max_occupancy", "rt.max_adults", "rt.max_children", "rpd.base_includes_children", "rp.cancellation_policy_id", "cp.policy_name as cancellation_policy_name", "cp.description as cancellation_policy_description", db.raw("MIN(ra.available_rooms) as available_rooms"), db.raw("MIN(rpd.base_rate) as min_base_rate"), db.raw("COALESCE(rpd.extra_adult_rate, 0) as extra_adult_rate"), db.raw("COALESCE(rpd.extra_child_rate, 0) as extra_child_rate"), "rpd.rate_plan_id", db.raw(`
                  JSON_AGG(
                ...
                `))
                    ...
                    this.on("rt.id", "=", "ra.room_type_id").andOn("ra.hotel_code", "=", "rt.hotel_code");
                })
                    .leftJoin("rate_plan_details as rpd", function () {
                    this.on("rt.id", "=", "rpd.room_type_id").andOn("rpd.hotel_code", "=", "rt.hotel_code");
                })
                    .leftJoin("rate_plans as rp", function () {
                    this.on("rpd.rate_plan_id", "=", "rp.id").andOn("rp.hotel_code", "=", "rt.hotel_code");
                })
                    ...
                    .groupBy("rt.id", "rt.name", "rt.base_occupancy", "rt.max_occupancy", "rt.max_adults", "rt.max_children", "rpd.extra_adult_rate", "rpd.rate_plan_id", "rpd.base_includes_children", "rp.cancellation_policy_id", "cp.policy_name", "cp.description", "rpd.extra_child_rate")
                    .havingRaw("COUNT(DISTINCT ra.date) = ?", [nights])
                    .havingRaw("MIN(ra.available_rooms) >= 1")
                    .havingRaw("MIN(rpd.base_rate) IS NOT NULL");
            });
            const buildRoomTypeResult = ({ single_room_type }, { searchedRooms, }) => __awaiter(this, void 0, void 0, function* () {
                var _a, _b;
                ...
                const dailyRates = yield fetchDailyRates(single_room_type.room_type_id, single_room_type.rate_plan_id, check_in, check_out, hotel_code);
                const baseRatePerDayList = [];
                for (let i = 0; i < nights; i++) {
                    baseRatePerDayList.push(Number(((_a = dailyRates[i]) === null || _a === void 0 ? void 0 : _a.rate) || baseRate));
                }
                let totalBaseRate = 0;
                ...
                const rateBreakdown = [];
                for (let i = 0; i < nights; i++) {
                    const date = (_b = dailyRates[i]) === null || _b === void 0 ? void 0 : _b.date;
                    ...
                    if ((accomodationSetting === null || accomodationSetting === void 0 ? void 0 : accomodationSetting.has_child_rates) &&
                        Array.isArray(searchedRooms.children_ages)) {
                        const getRateForAge = (age) => {
                            const rateGroup = childRateGroups.find((r) => r.room_type_id === single_room_type.room_type_id &&
                                r.rate_plan_id === single_room_type.rate_plan_id &&
                                age >= r.age_from &&
                                age <= r.age_to);
                            if (!rateGroup)
                                return fallbackExtraChildRate;
                        ...
                        if (baseIncludesChildren) {
                            ...
                            const extraChildAges = searchedRooms.children_ages.slice(baseChildCount);
                ...
                return Object.assign(Object.assign({}, single_room_type), { matched_occupancy: searchedRooms, daily_rates_used: baseRatePerDayList.some((_, i) => dailyRates[i]), total_price: totalPrice.toFixed(2), rate_breakdown: rateBreakdown });
            });
            let matchedRoomTypes = [];
            if (isAllSameConfig) {
                const { adults, children } = rooms[0];
                const roomTypes = yield fetchMatchingRoomTypes(adults, children);
                matchedRoomTypes = yield Promise.all(roomTypes
                    .filter((rt) => parseInt(rt.available_rooms) >= totalRoomsRequested)
                    .map((rt) => buildRoomTypeResult({ single_room_type: rt }, { searchedRooms: rooms[0] })));
            }
            else {
                matchedRoomTypes = [];
                const tempResults = [];
                for (const searchedRoom of rooms) {
                    const roomTypes = yield fetchMatchingRoomTypes(searchedRoom.adults, searchedRoom.children);
                    const matching = roomTypes
                        .filter((rt) => parseInt(rt.available_rooms) >= 1)
                        .map((rt) => buildRoomTypeResult({ single_room_type: rt }, { searchedRooms: searchedRoom }));
                    tempResults.push(...matching);
                }
                matchedRoomTypes = yield Promise.all(tempResults);
            }
            return {
                success: true,
