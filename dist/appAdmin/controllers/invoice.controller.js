...
class InvoiceController extends abstract_controller_1.default {
    constructor() {
        ...
        this.getAllFolioInvoice = this.asyncWrapper.wrap({
            querySchema: this.validator.getAllFolioValidator,
        }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _b = yield this.service.getAllFolioInvoice(req), { code } = _b, data = __rest(_b, ["code"]);
            res.status(code).json(data);
        }));
        this.getSingleFolioInvoice = this.asyncWrapper.wrap({
            paramSchema: this.commonValidator.singleParamValidator(),
        }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _c = yield this.service.getSingleFolioInvoice(req), { code } = _c, data = __rest(_c, ["code"]);
            res.status(code).json(data);
        }));
        this.deleteSingleFolioInvoice = this.asyncWrapper.wrap({
            paramSchema: this.commonValidator.singleParamValidator(),
        }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _d = yield this.service.deleteSingleFolioInvoice(req), { code } = _d, data = __rest(_d, ["code"]);
            res.status(code).json(data);
        }));
        this.getSingleBookingRoomsInvoice = this.asyncWrapper.wrap({
            paramSchema: this.commonValidator.singleParamValidator(),
        }, (req, res) => __awaiter(this, void 0, void 0, function* () {
            const _e = yield this.service.getSingleBookingRoomsInvoice(req), { code } = _e, data = __rest(_e, ["code"]);
            res.status(code).json(data);
        }));
    }
}
